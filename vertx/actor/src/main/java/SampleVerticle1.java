...
public class SampleVerticle1 extends AbstractVerticle {
  ...
  public void start()
  {
      ...
      System.out.printf("Verticle1 Slot_0: %s%n", map.get(ClientVerticle.SLOT0));

      map.put(ClientVerticle.SLOT1, "V1: " + Thread.currentThread());

      vertx.eventBus().send(
              "sample.data",
              "hello from SampleVerticle1",
              r -> {
                  System.out.println("[SampleVerticle1] Receiving reply ' " + r.result().body()
