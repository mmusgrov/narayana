...
public class SampleVerticle2 extends AbstractVerticle {
  ...
  public void start()
  {
      ...
      System.out.printf("Verticle2 Slot_0: %s%n", map.get(ClientVerticle.SLOT0));

      map.put(ClientVerticle.SLOT2, "V2: " + Thread.currentThread());

      vertx.eventBus().send(
              "sample.data",
              "hello from SampleVerticle2",
              r -> {
                  System.out.println("[SampleVerticle2] Receiving reply ' " + r.result().body()
