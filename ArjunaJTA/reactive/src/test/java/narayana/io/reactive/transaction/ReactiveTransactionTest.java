...
public class ReactiveTransactionTest {
    ...
    @Test
    // test that XAResources are handled correctly
    public void testWithResourceEnlistmentFailures() {
        String expected = "code block response";
        Function<ReactiveTransaction, Uni<String>> work = txn -> {
            out.printf("transactional work with txn=%s%n", txn);
            return Uni.createFrom().item(expected);
        ...
        TransactionManagerImpl tm = new TransactionManagerImpl();
        DummyXA xar0 = new DummyXA(false);
        // xar1 will fail the enlist/start call forcing the transaction to roll back
        DummyXA xar1 = new DummyXA(false, true);

        String actual = tm
                .withTransaction(work, xar0, xar1)
                .onFailure().invoke(e -> fail(e.getMessage()))
                .await().indefinitely();

        assertEquals(expected, actual);

        // prepare/commit will not be called since the transaction is marked rollback only
        assertEquals(0, xar0.prepareCnt.get());
        assertEquals(0, xar1.prepareCnt.get());
        assertEquals(0, xar0.commitCnt.get());
        assertEquals(0, xar1.commitCnt.get());
        // xar0 was enlisted so should have been asked to rollback
        assertEquals(1, xar0.rollbackCnt.get());
        assertEquals(0, xar1.rollbackCnt.get()); // xar1 should not have been enlisted
    }
    ...
    // I wanted to add that but could not create a test for it since Mutiny seems to always run my
    // "Uni flow" in a single thread. The next test is my attempt to get two flows to interleave but
    // it seems to pass without suspend/resume. Any help the reader may provide here would be appreciated.
    public void testTransactionsDoNotInterleave() throws InterruptedException {
        ...
        assertEquals("the first transaction did not commit successfully",
                Status.STATUS_COMMITTED, synchronizations[0].status);
        assertEquals("the second transaction did not commit successfully",
                Status.STATUS_COMMITTED, synchronizations[1].status);
