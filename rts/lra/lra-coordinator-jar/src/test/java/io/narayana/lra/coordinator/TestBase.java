...
public abstract class TestBase {
    ...
    public void before() throws URISyntaxException, MalformedURLException {
        LRALogger.logger.infof("Starting test %s", testName);
    ...
    @After
    public void after() {
        LRALogger.logger.infof("Finised test %s", testName);
        lraClient.close();
    }
    ...
    void restartContainer() {
        try {
            ...
            System.out.printf("jboss-as kill worked%n");
        } catch (Exception e) {
            System.out.printf("jboss-as kill: %s", e.getMessage());
        }
