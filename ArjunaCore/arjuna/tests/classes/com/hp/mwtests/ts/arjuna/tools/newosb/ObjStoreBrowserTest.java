...
//		}
	...
	public void aaTest(boolean replay) throws Exception {
		...
		assertNotNull(w);
//xx		OSEntryBean ai = w.getMBean();
//xx		assertNotNull(ai);

		// the MBean should wrap an ActionBean
//xx		assertTrue(ai instanceof ActionBeanHandler);
//TODO		ActionBean actionBean = (ActionBeanHandler) ai;

		Set<LogRecordWrapperMXBean> participants = osb.findParticipants(w);

		// and there should be one MBean corresponding to the CrashRecord that got the heuristic:
		assertEquals(1, participants.size());
		LogRecordWrapperMXBean lw = participants.iterator().next();
