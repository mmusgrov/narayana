 ...
 * Unlike {@link JGroupsClusterMemberId}, this generator creates keys based only on the
 * group name and slot index, not node ID or random UIDs.
 *
...
public class SharedSlotKeyGenerator implements JGroupsSlotKeyGenerator {
    ...
    public ByteArrayKey generateUniqueKey(int index) {
        // Create deterministic key: {groupName};slot;{index}
        try {
