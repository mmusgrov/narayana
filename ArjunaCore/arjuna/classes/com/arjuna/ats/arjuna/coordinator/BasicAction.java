...
public class BasicAction extends StateManager
{
    ...
    private void traceEndStatus() {
        ...
        if (failedList.size() != 0)
            for (AbstractRecord rec = failedList.peekFront(); rec != null; rec = failedList.peekNext(rec))
                sb.append(rec.get_uid()).append(' ');

        tsLogger.logger.tracef("BasicAction::End() result for action-id (%s) "
            + "is (%d). Failed List: (%s). Node id: (%s)",
            get_uid(),
            status,
            sb.toString(),
