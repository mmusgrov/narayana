...
public class BasicAction extends StateManager
{
                ...
                else
                {
                    ...
                    if (prepareStatus == TwoPhaseOutcome.PREPARE_NOTOK
                        ...
                        if (!reportHeuristics && TxControl.asyncCommit
                            ...
                            doAsynchronousCommit(this::phase2Commit, false);
                        } else
                    ...
                    else
                    {
                        if (!reportHeuristics && TxControl.asyncCommit
                                && (parentAction == null))
                        {
                            // running async so ignore heuristic info
                            doAsynchronousCommit(this::phase2Commit, false);
                        }
                        else
    ...
    {
        synchronizationLock.lock();
        System.out.printf("java21: phase2Commit isVirtual=%b\n", Thread.currentThread().isVirtual());
        try {
            if (tsLogger.logger.isTraceEnabled()) {
     ...
     */
    protected final void doAsynchronousCommit(java.util.function.Function<Boolean, Void> func, boolean reportHeuristics) {
        try (var executor = java.util.concurrent.Executors.newVirtualThreadPerTaskExecutor()) {
            executor.submit(
                    () -> func.apply(reportHeuristics)
            ).get();
        } catch (ExecutionException | InterruptedException e) {
            throw new RuntimeException(e);
        }
    }
    protected final Future<Integer> doAsynchronousPrepare(BiFunction<Boolean, AbstractRecord, Integer> func,
    ...
    protected int async_prepare(boolean reportHeuristics) {
        ...
        while (pendingList.size() != 0) {
            tasks.add(doAsynchronousPrepare(this::doPrepare, pendingList.getFront(), reportHeuristics));
        }
    ...
    {
        System.out.printf("java21: doPrepare isVirtual=%b\n", Thread.currentThread().isVirtual());        /*
         * Here is the start of the hard work. Walk down the pendingList
         * invoking the appropriate prepare operation. If it succeeds put the
         * record on either the preparedList or the read_only list and continue
         * until the pendingList is exhausted.
